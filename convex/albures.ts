...
export const submitScore = mutation({
    ...
    handler: async (ctx, { userId, score }) => {
        ...
        const existing = await ctx.db.query("alburesScores").withIndex("by_user", (q) => q.eq("userId", userId)).first();
        if (!existing) {
            await ctx.db.insert("alburesScores", { userId, allTimeBest: score, dailyBest: score, dailyDate: today, weeklyBest: score, weeklyStr: week });
            return;
        }
...
export const getLeaderboard = query({
    ...
    handler: async (ctx, { type }) => {
        ...
        if (type === "alltime") {
            const e = await ctx.db.query("alburesScores").withIndex("by_alltime").order("desc").take(20);
            rows = e.map((x) => ({ userId: x.userId, score: x.allTimeBest }));
        } else if (type === "daily") {
            const e = await ctx.db.query("alburesScores").withIndex("by_daily", (q) => q.eq("dailyDate", today)).order("desc").take(20);
            rows = e.map((x) => ({ userId: x.userId, score: x.dailyBest }));
        } else {
            const e = await ctx.db.query("alburesScores").withIndex("by_weekly", (q) => q.eq("weeklyStr", week)).order("desc").take(20);
...
export const getMyBest = query({
    ...
    handler: async (ctx, { userId }) => {
        ...
        const rec = await ctx.db.query("alburesScores").withIndex("by_user", (q) => q.eq("userId", userId)).first();

...
function getKeyboardLayoutMetrics({ layout, portraitHeight, portraitKeyH } = {}) {
  ...
  const showSpecialLabels = layout.mode !== 'compact';
  const showKeyboardHint = layout.mode === 'phone' || layout.mode === 'tablet';
  const keyboardHintHeight = showKeyboardHint ? 18 : 0;
  const powerUpHeight = availableHeight < 260
    ? 60
    : layout.mode === 'tablet' || layout.mode === 'landscape' ? 76 : 64;
  const minimumKeyboardHeight = kbPaddingV * 2 + 36 * 3 + kbRowMarginB * 3 + keyboardHintHeight;
  const minimumControlsHeight = powerUpHeight + minimumKeyboardHeight;
  const desiredHeight = portraitHeight
    ?? powerUpHeight + kbPaddingV * 2 + layout.keyHeight * 3 + kbRowMarginB * 3 + keyboardHintHeight;
  ...
  const maximumKeyHeight = Math.max(
    0,
    Math.floor((kbHeight - powerUpHeight - kbPaddingV * 2 - kbRowMarginB * 3 - keyboardHintHeight) / 3),
  );
  const desiredKeyHeight = portraitKeyH ?? layout.keyHeight;
  const kbKeyH = Math.min(desiredKeyHeight, maximumKeyHeight);
  const keyboardHeight = kbPaddingV * 2 + kbKeyH * 3 + kbRowMarginB * 3 + keyboardHintHeight;
  ...
  return {
    ...
    showKeyboardHint,
    keyboardHintHeight,
    specialLabelFontSize: tabletScale ? 9 : 8,
  };
}
